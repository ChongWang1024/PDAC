...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, noise_level):
        ...
        noise_level = self.fc(noise_level)
        encoding = noise_level * torch.exp(-math.log(1e4) * step.unsqueeze(0))
        encoding = torch.cat(
...
class SwinTransformerBlock_with_AdaLN(nn.Module):
    r""" Swin Transformer Block.
    Args:
        dim (int): Number of input channels.
        input_resolution (tuple[int]): Input resulotion.
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.adaLN_modulation = nn.Sequential(
            # nn.Linear(66, dim, bias=True),
            nn.SiLU(),
            nn.Linear(66, 6 * dim, bias=True)
        )
    ...
    def forward(self, x, t, x_size):
        H, W = x_size
        B, L, C = x.shape
        # assert L == H * W, "input feature has wrong size"

        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(t).chunk(6, dim=1)       #  AdaLN

        shortcut = x
        x = self.norm1(x)

        x = modulate(x, shift_msa, scale_msa)       #  AdaLN
        ...
        x = x * gate_msa.unsqueeze(1)       #  AdaLN

        # FFN
        x = shortcut + self.drop_path(x)
        x = x + self.drop_path(self.mlp(modulate(self.norm2(x), shift_mlp, scale_mlp)) * gate_mlp.unsqueeze(1))       #  AdaLN
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock_with_AdaLN(dim=dim, input_resolution=input_resolution,
    ...
    def forward(self, x, t, x_size):
        for blk in self.blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, t, torch.tensor(x_size))
            else:
                x = blk(x, t, x_size)
...
class HUMUSBlock_pdac(nn.Module):
    r""" HUMUS-Block
    Args:
    ...
    def __init__(self, 
                 ...
                 **kwargs):
        super(HUMUSBlock_pdac, self).__init__()
        num_in_ch = in_chans
        ...
        #####################################################################################################
        ################################### 0, Degradation Severity Embedding block ###################################
    ...
    def forward_features(self, x, t):
        ...
        for i, layer in enumerate(self.layers_down):
            x, skip = layer(x, t, x_size=layer.input_resolution)
            skip_cons.append(skip)
        x = self.norm_down(x)  # B L C
        
        # bottleneck
        x = self.layer_bottleneck(x, t, self.layer_bottleneck.input_resolution)
        
        # decode
        for i, layer in enumerate(self.layers_up):
            x = layer(x, t, x_size=layer.input_resolution, skip=skip_cons[-i-1])
        x = self.norm_up(x)
        x = self.patch_unembed(x, x_size)
        return x

    def forward(self, x, prob_mask):
        C, H, W = x.shape[1:]
        center_slice = (C - 1) // 2
        x = self.check_image_size(x)

        t = self.mask_embedder(prob_mask)
...
class MaskPredictor(nn.Module):
    ...
    def __init__(self):
        ...
        self.predictor = nn.Sequential(
            MaskPatchEmbedLearned(img_size=384, patch_size=24, in_chans=2, embed_dim=384, norm_layer=nn.LayerNorm),
            MaskPatchMerging(input_resolution=[16, 16], dim=384, norm_layer=nn.LayerNorm),
            MaskPatchMerging(input_resolution=[8, 8], dim=384, norm_layer=nn.LayerNorm),
            MaskPatchMerging(input_resolution=[4, 4], dim=384, norm_layer=nn.LayerNorm),
            MaskPatchMerging(input_resolution=[2, 2], dim=384, norm_layer=nn.LayerNorm),
            nn.Sigmoid()
        )

    def forward(self, kspace: torch.Tensor) -> torch.Tensor:
        prob_mask = self.predictor(kspace)

        return prob_mask
    

class MaskPatchEmbedLearned(nn.Module):
    r""" Image to Patch Embedding with arbitrary patch size and learned linear projection.
    Args:
        img_size (int): Image size.  
        patch_size (int): Patch token size. 
...
class MaskPatchMerging(nn.Module):
    r""" Patch Merging Layer.
    Args:
        input_resolution (tuple[int]): Resolution of input feature.
        dim (int): Number of input channels.
