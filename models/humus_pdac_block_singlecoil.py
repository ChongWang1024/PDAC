...
class HUMUSBlock_pdac_singlecoil(nn.Module):
    r""" HUMUS-Block
    Args:
    ...
    def __init__(self, 
                 ...
                 **kwargs):
        super(HUMUSBlock_pdac_singlecoil, self).__init__()
        num_in_ch = in_chans
...
class MaskPredictor(nn.Module):
    ...
    def __init__(self):
        ...
        self.predictor = nn.Sequential(
            MaskPatchEmbedLearned(img_size=320, patch_size=20, in_chans=2, embed_dim=320, norm_layer=nn.LayerNorm),
            MaskPatchMerging(input_resolution=[16, 16], dim=320, norm_layer=nn.LayerNorm),
            MaskPatchMerging(input_resolution=[8, 8], dim=320, norm_layer=nn.LayerNorm),
            MaskPatchMerging(input_resolution=[4, 4], dim=320, norm_layer=nn.LayerNorm),
            MaskPatchMerging(input_resolution=[2, 2], dim=320, norm_layer=nn.LayerNorm),
            nn.Sigmoid()
        )
